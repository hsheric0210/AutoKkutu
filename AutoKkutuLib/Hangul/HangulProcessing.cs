...
public static class HangulProcessing
{
	...
	public static string AppendChar(this string str, JamoType type, char ch)
	{
		...
		var result = ch;
		return lastSplit?.IsHangul == true ? CombineHangulSebeol(str, type, ch, lastSplit) : str + ch;
	}
	...
	{
		return HangulConstants.InverseConsonantClusterTable.TryGetValue(consonantCluster, out IList<char>? consonants)
			? consonants
